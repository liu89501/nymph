...
public abstract class JSONUtil {
	...
	public static String resolve(Object object) {
		...
		try {
			...
			if (object instanceof Collection) {
				return collectionToJson((Collection<?>) object, integer);
			} else if (object instanceof Map) {
				return mapToJson((Map<String, String>) object);
			} else {
				return commonToJson(object, integer);
			}
		} catch (Exception e) {
			e.printStackTrace();
			return "null";
	...
	private static String commonToJson(Object json, AtomicInteger integer) throws Exception {
		...
		for (Field field : fields) {
			...
			FmtName name;
			if ((name = field.getAnnotation(FmtName.class)) != null) {
				fName = name.value();
			}
			...
			if (isCommonType(field)) {
				...
				if (field.isAnnotationPresent(FmtDate.class)) {
					FmtDate dateFmt = field.getAnnotation(FmtDate.class);
					resolve = DateUtils.resolve(date, dateFmt.value());
				} else {
				...
				jsons.append(collectionToJson((Collection<?>) field.get(json), integer));

			} else {
				jsons.append(commonToJson(field.get(json), integer));
			}
		}
		return jsons.delete(1, 2).append("}").toString();
	}

	private static String collectionToJson(Collection<?> coll, AtomicInteger integer) throws Exception {
		if (integer.incrementAndGet() >= 10)
			return null;

		StringBuilder jsons = new StringBuilder("[");
		for (Object object : coll) {
			jsons.append(",").append(commonToJson(object, integer));
		}
		return jsons.delete(1, 2).append("]").toString();
	}
